...
if __name__ == '__main__':
    ...
    # SQLContext = SQL({'server': 'wpdb2.hihosting.hinet.net', 'user': 'p89880749_p89880749',
    ...
    # data = [value]
    # err = sqlContext.insert('Bingo', data)
    # print(err)
    # selectSQL = "select * from Bingo where dDate != \'2024-02-16\'"
    # selectSQL = "select * from Bingo "
    # rows = SQLContext.select(selectSQL)

    # print("筆數" + str(len(rows)))
    # for row in rows:
    ...
    # MS-SQL 資料庫連線設定
    db_settings = {
        'server': 'wpdb2.hihosting.hinet.net',
        'user': 'p89880749_p89880749',
        'password': 'Jonny1070607!@#$%',
        'database': 'p89880749_test'
    }
  ...
  # 將資料寫入 CSV
    output_file = 'bingo_ball.csv'
    with open(output_file, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(
            file, fieldnames=[col[0] for col in cursor.description])
        writer.writeheader()
        for row in cursor:
            writer.writerow(row)
